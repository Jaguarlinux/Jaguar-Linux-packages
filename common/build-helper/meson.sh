...
if [[ "${build_helper}" = *qemu* ]]; then
	_MESON_EXE_WRAPPER="exe_wrapper = '/usr/bin/qemu-${DULGE_TARGET_QEMU_MACHINE}'"
fi
...
cat > "${DULGE_WRAPPERDIR}/meson/dulge_meson.cross" <<-EOF
	[binaries]
	${_MESON_EXE_WRAPPER:-# exe_wrapper is not set}
	c = '${CC}'
	cpp = '${CXX}'
	ar = '${DULGE_CROSS_TRIPLET}-gcc-ar'
	nm = '${NM}'
	strip = '${STRIP}'
	readelf = '${READELF}'
	objcopy = '${OBJCOPY}'
	pkg-config = '${PKG_CONFIG}'
	rust = ['rustc', '--target', '${RUST_TARGET}' ,'--sysroot', '${DULGE_CROSS_BASE}/usr']
	g-ir-scanner = '${DULGE_CROSS_BASE}/usr/bin/g-ir-scanner'
	g-ir-compiler = '${DULGE_CROSS_BASE}/usr/bin/g-ir-compiler'
	g-ir-generate = '${DULGE_CROSS_BASE}/usr/bin/g-ir-generate'
	llvm-config = 'llvm-config-qemu'
	cups-config = '${DULGE_CROSS_BASE}/usr/bin/cups-config'

	[properties]
	needs_exe_wrapper = true
	bindgen_clang_arguments = ['-target', '${DULGE_CROSS_TRIPLET}']
