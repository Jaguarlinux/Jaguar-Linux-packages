...
CFLAGS+=" -I${DULGE_CROSS_BASE}/${py3_sitelib}/numpy/_core/include"
LDFLAGS+=" -L${DULGE_CROSS_BASE}/${py3_sitelib}/numpy/_core/lib"
LDFLAGS+=" -L${DULGE_CROSS_BASE}/${py3_sitelib}/numpy/random/lib"

# distutils from python3-numpy looks to environment variables F77 and
# F90 rather than the DULGE-set FC
export F77="${FC}"
export F90="${FC}"
...
if _gfortran=$(command -v "${FC}"); then
	ln -sf "${_gfortran}" "${DULGE_WRAPPERDIR}/gfortran"
fi
unset _gfortran

# Write a secondary meson cross file for numpy configuration
if [[ "${build_helper}" = *meson* ]]; then
	_npy_meson_cross="${DULGE_WRAPPERDIR}/meson/dulge_numpy.cross"
	_cross_py_site="${DULGE_CROSS_BASE}/${py3_sitelib}"

	if [ ! -e "${_npy_meson_cross}" ] || [ -n "$DULGE_BUILD_FORCEMODE" ]; then
