# This hook checks for common issues related to jaguar.

hook() {
	local error=0 filename= rev= libname= conflictPkg= conflictFile=
	local conflictRev= ignore= found= mapshlibs=$DULGE_COMMONDIR/shlibs
	local emptypkg=yes

	set +E

	# Check for forbidden directories that are symlinks in jaguar.
	...
	if [ ! -s "${DULGE_STATEDIR}/${pkgname}-shlib-provides" ]; then
		return 0
	fi

	for filename in $(<"${DULGE_STATEDIR}/${pkgname}-shlib-provides"); do
		rev=${filename#*.so.}
		libname=${filename%.so*}
		...
		grep -E "${_pattern}" $mapshlibs | { \
			while read -r conflictFile conflictPkg ignore; do
				...
				if [ -n "$ignore" -a "$ignore" != "$DULGE_TARGET_MACHINE" ]; then
					continue
				...
				_revdeps=$($DULGE_QUERY_XCMD -Rs ${libname}.so -p shlib-requires|cut -d ' ' -f1)
			...
			for f in ${provides}; do
				_vpkgname="$($DULGE_UHELPER_CMD getpkgname ${f} 2>/dev/null)"
				_spkgname="$(grep "^${filename}" $mapshlibs | cut -d ' ' -f2)"
				_libpkgname="$($DULGE_UHELPER_CMD getpkgname ${_spkgname} 2>/dev/null)"
