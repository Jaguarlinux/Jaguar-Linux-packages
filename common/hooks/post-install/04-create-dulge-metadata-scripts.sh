# This hook generates DULGE pkg metadata INSTALL/REMOVE scripts.
...
TRIGGERSDIR="./usr/libexec/dulge-triggers"
ACTION="\$1"
PKGNAME="\$2"
VERSION="\$3"
UPDATE="\$4"
...
_EOF
	...
	if [ -n "$triggers" ]; then
		...
		for f in ${triggers}; do
			targets=$($DULGE_TRIGGERSDIR/$f targets)
			for j in ${targets}; do
...
hook() {
	...
	if [ -n "${sourcepkg}" -a "${sourcepkg}" != "${pkgname}" ]; then
		# subpkg
		meta_install=${DULGE_SRCPKGDIR}/${pkgname}/${pkgname}.INSTALL
		msg_install=${DULGE_SRCPKGDIR}/${pkgname}/${pkgname}.INSTALL.msg
		meta_remove=${DULGE_SRCPKGDIR}/${pkgname}/${pkgname}.REMOVE
		msg_remove=${DULGE_SRCPKGDIR}/${pkgname}/${pkgname}.REMOVE.msg
	else
		# sourcepkg
		meta_install=${DULGE_SRCPKGDIR}/${pkgname}/INSTALL
		msg_install=${DULGE_SRCPKGDIR}/${pkgname}/INSTALL.msg
		meta_remove=${DULGE_SRCPKGDIR}/${pkgname}/REMOVE
		msg_remove=${DULGE_SRCPKGDIR}/${pkgname}/REMOVE.msg
	fi
