...
#	- generates shlib-provides file for dulge-create(1)
...
collect_sonames() {
	...
	if [ -s "${_tmpfile}" ]; then
		tr '\n' ' ' < "${_tmpfile}" > "${DULGE_STATEDIR}/${pkgname}${_suffix}-shlib-provides"
		echo >> "${DULGE_STATEDIR}/${pkgname}${_suffix}-shlib-provides"
		if [ "$_mainpkg" ]; then
...
hook() {
	local _destdir32=${DULGE_DESTDIR}/${pkgname}-32bit-${version}
	local _mainpkg=yes
	local _pkg
