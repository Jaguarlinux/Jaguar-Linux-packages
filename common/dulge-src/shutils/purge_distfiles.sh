...
purge_distfiles() {
	readonly HASHLEN=64
	if [ -z "$DULGE_SRCDISTDIR" ]; then
		msg_error "The variable \$DULGE_SRCDISTDIR is not set."
		exit 1
	fi
	#
	...
	#
	# Collect inodes of all distfiles in $DULGE_SRCDISTDIR
	#
	declare -A inodes
	distfiles=($DULGE_SRCDISTDIR/*/*)
	max=${#distfiles[@]}
	if [ -z "$max" ]; then
		msg_error "No distfiles files found in '$DULGE_SRCDISTDIR'"
		exit 1
	fi
	cur=0
	percent=-1
	...
	hashes=($DULGE_SRCDISTDIR/by_sha256/*)
