...
show_pkg_deps() {
    [ -f "${DULGE_STATEDIR}/${pkgname}-rdeps" ] && cat "${DULGE_STATEDIR}/${pkgname}-rdeps"
}
...
show_avail() {
    check_pkg_arch "$DULGE_CROSS_BUILD" 2>/dev/null
}
...
show_eval_dep() {
    ...
    if [ ! -f $DULGE_SRCPKGDIR/$_dep/template ]; then
    ...
    [[ $found ]] && return
    _srcpkg=$(readlink -f ${DULGE_SRCPKGDIR}/${_dep})
    _srcpkg=${_srcpkg##*/}
    echo $_srcpkg
}
...
show_pkg_build_options() {
    ...
    source $DULGE_COMMONDIR/options.description
...
show_pkg_shlib_provides() {
    [ -f "${DULGE_STATEDIR}/${pkgname}-shlib-provides" ] && cat "${DULGE_STATEDIR}/${pkgname}-shlib-provides"
}

show_pkg_shlib_requires() {
    [ -f "${DULGE_STATEDIR}/${pkgname}-shlib-requires" ] && cat "${DULGE_STATEDIR}/${pkgname}-shlib-requires"
}
