...
setup_pkg_depends() {
    ...
    for j in ${depends}; do
        ...
        if [[ ${_rpkgname} == virtual ]]; then
            _pkgname=$(dulge-uhelper getpkgname $_depname 2>/dev/null)
            [ -z "$_pkgname" ] && _pkgname="$_depname"
            if [ -s ${DULGE_DISTDIR}/etc/virtual ]; then
                foo=$(grep -E "^${_pkgname}[[:blank:]]" ${DULGE_DISTDIR}/etc/virtual|cut -d ' ' -f2)
            elif [ -s ${DULGE_DISTDIR}/etc/defaults.virtual ]; then
                foo=$(grep -E "^${_pkgname}[[:blank:]]" ${DULGE_DISTDIR}/etc/defaults.virtual|cut -d ' ' -f2)
            fi
            if [ -z "$foo" ]; then
            ...
            foo="$($DULGE_UHELPER_CMD getpkgdepname ${_depname} 2>/dev/null)"
            if [ -z "$foo" ]; then
                foo="$($DULGE_UHELPER_CMD getpkgname ${_depname} 2>/dev/null)"
                [ -z "$foo" ] && foo="${_depname}"
            fi
            [[ $out ]] && echo "$foo"
        fi
...
# EBUSY   (16): package 'dulge' needs to be updated.
# EEXIST  (17): file conflicts in transaction (DULGE_FLAG_IGNORE_FILE_CONFLICTS unset)
...
install_pkg_from_repos() {
    ...
    case "$rval" in
        0) # success, check if there are errors.
           ...
           ;;
        *)
           [ -z "$DULGE_KEEP_ALL" ] && remove_pkg_autodeps
...
check_installed_pkg() {
    ...
    pkgn="$($DULGE_UHELPER_CMD getpkgname ${pkg})"
    [ -z "$pkgn" ] && return 2

    uhelper=$DULGE_UHELPER_CMD
    [[ $cross ]] && uhelper=$DULGE_UHELPER_XCMD
    iver="$($uhelper version $pkgn)"
    if [ $? -eq 0 -a -n "$iver" ]; then
        $DULGE_CMPVER_CMD "${pkgn}-${iver}" "${pkg}"
...
install_pkg_deps() {
    ...
    if [ "$pkg" != "$targetpkg" ]; then
        msg_normal "$pkgver: building${style} (dependency of $targetpkg) for $DULGE_TARGET_MACHINE...\n"
    else
        msg_normal "$pkgver: building${style} for $DULGE_TARGET_MACHINE...\n"
    fi
    ...
    if [[ ${hostmakedepends} ]]; then
        ...
        for f in ${hostmakedepends}; do
            if [ -f $DULGE_SRCPKGDIR/$f/template ]; then
                templates+=" $f"
                continue
            fi
            local _repourl=$($DULGE_QUERY_CMD -R -prepository "$f" 2>/dev/null)
            if [ "$_repourl" ]; then
        ...
        done < <($DULGE_CHECKVERS_CMD -D $DULGE_DISTDIR -sm $templates)
    fi
    ...
    if [[ ${makedepends} ]]; then
        ...
        for f in ${makedepends}; do
            ...
            local _repourl=$($DULGE_QUERY_XCMD -R -prepository "$f" 2>/dev/null)
            if [ "$_repourl" ]; then
        ...
        done < <($DULGE_CHECKVERS_XCMD -D $DULGE_DISTDIR -sm $templates)
    fi
    ...
    if [ -n "$DULGE_BUILD_ONLY_ONE_PKG" ]; then
    ...
    for i in ${host_missing_deps[@]}; do
        ...
        (
        curpkgdepname=$($DULGE_UHELPER_CMD getpkgname "$i" 2>/dev/null)
        setup_pkg $curpkgdepname
        # do not check when building dependencies, except for "full" (-K)
        [ "$DULGE_CHECK_PKGS" == full ] || unset DULGE_CHECK_PKGS
        exec env DULGE_DEPENDENCY=1 DULGE_BINPKG_EXISTS=1 DULGE_DEPENDS_CHAIN="$DULGE_DEPENDS_CHAIN, $sourcepkg(host)" \
            $DULGE_LIBEXECDIR/build.sh $sourcepkg $pkg $target $cross_prepare || exit $?
        ) || exit $?
        host_binpkg_deps+=("$i")
    done
    ...
    for i in ${missing_deps[@]}; do
        ...
        exec env DULGE_DEPENDENCY=1 DULGE_BINPKG_EXISTS=1 DULGE_DEPENDS_CHAIN="$DULGE_DEPENDS_CHAIN, $sourcepkg(${cross:-host})" \
            $DULGE_LIBEXECDIR/build.sh $sourcepkg $pkg $target $cross $cross_prepare || exit $?
        ) || exit $?
        binpkg_deps+=("$i")
    done
    ...
    for i in ${missing_rdeps[@]}; do
        # packages not found in repos, install from source.
        (
        curpkgdepname=$($DULGE_UHELPER_CMD getpkgdepname "$i" 2>/dev/null)
