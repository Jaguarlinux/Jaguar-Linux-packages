...
update_check() {
    ...
    local update_override=$DULGE_SRCPKGDIR/$DULGE_TARGET_PKG/update
    ...
    local curlargs=(
        -A "dulge-src-update-check/$DULGE_SRC_VERSION"
        --max-time 10 --compressed -Lsk
    )

    pkgname=${pkgname#kf6-}

    # DULGE_UPDATE_CHECK_VERBOSE is the old way to show verbose messages
    [ "$DULGE_UPDATE_CHECK_VERBOSE" ] && DULGE_VERBOSE="$DULGE_UPDATE_CHECK_VERBOSE"

    if [ -r $update_override ]; then
        . $update_override
        msg_verbose "using $DULGE_TARGET_PKG/update overrides\n"
        if [ -n "$disabled" ]; then
    ...
    while IFS= read -r found_version; do
        ...
        if $consider; then
            dulge-uhelper cmpver "$original_pkgname-${version}_1" \
