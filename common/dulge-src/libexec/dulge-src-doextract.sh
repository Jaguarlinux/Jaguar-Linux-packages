...
for f in $DULGE_COMMONDIR/environment/extract/*.sh; do
    source_file "$f"
done

DULGE_EXTRACT_DONE="${DULGE_STATEDIR}/${sourcepkg}_${DULGE_CROSS_BUILD}_extract_done"

if [ -f $DULGE_EXTRACT_DONE ]; then
    exit 0
fi
...
if declare -f do_extract >/dev/null; then
    ...
    if [ -n "$build_style" ]; then
        if [ ! -r $DULGE_BUILDSTYLEDIR/${build_style}.sh ]; then
            msg_error "$pkgver: cannot find build helper $DULGE_BUILDSTYLEDIR/${build_style}.sh!\n"
        fi
