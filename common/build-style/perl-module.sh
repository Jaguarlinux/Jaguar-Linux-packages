...
# than $DULGE_BUILDDIR/$wrksrc/$build_wrksrc, one can use (relative to
# $wrksrc/$build_wrksrc):
#
...
do_configure() {
	...
	local perlprefix=${DULGE_STATEDIR}/perlprefix-${DULGE_TARGET_MACHINE}
	mkdir -p $perlprefix
	if [ -d "$DULGE_CROSS_BASE/usr/lib/perl5/core_perl" ]; then
		cp "$DULGE_CROSS_BASE/usr/lib/perl5/core_perl/Config"*.p? $perlprefix
		cp "$DULGE_CROSS_BASE/usr/lib/perl5/core_perl/Errno.pm" $perlprefix
		sed -i -e "s;archlibexp => '\(.*\)';archlibexp => '${DULGE_CROSS_BASE}\1';" \
	...
	if [ -f "${wrksrc}/${build_wrksrc:+$build_wrksrc/}Makefile.PL" ]; then
		sed -i "s,/usr/include,${DULGE_CROSS_BASE}/usr/include,g" \
	...
	if [ -z "$perl_configure_dirs" ]; then
		...
		PERL_MM_USE_DEFAULT=1 GCC="$CC" CC="$CC" LD="$CC" \
			OPTIMIZE="$CFLAGS" \
			CFLAGS="$CFLAGS -I${DULGE_CROSS_BASE}/usr/include" \
			LDFLAGS="$LDFLAGS -L${DULGE_CROSS_BASE}/usr/lib -lperl" \
			LDDLFLAGS="-shared $CFLAGS -L${DULGE_CROSS_BASE}/usr/lib" \
	...
	for i in ${perl_configure_dirs}; do
		...
		if [ -f "$perlmkf" ]; then
			...
			PERL_MM_USE_DEFAULT=1 GCC="$CC" CC="$CC" LD="$CC" \
				...
				CFLAGS="$CFLAGS -I${DULGE_CROSS_BASE}/usr/include" \
				LDFLAGS="$LDFLAGS -L${DULGE_CROSS_BASE}/usr/lib -lperl" \
				LDDLFLAGS="-shared $CFLAGS -L${DULGE_CROSS_BASE}/usr/lib -lperl" \
