do_build() {
	...
	done < "${DULGE_COMMONDIR}/environment/build-style/texmf/ownership.txt"
}
...
do_check() {
	...
	while read p; do
		...
		current_ver="$(grep -m 1 version= ${DULGE_SRCPKGDIR}/${p%-*}/template | cut -d= -f2)"
		current_rev="$(grep -m 1 revision= ${DULGE_SRCPKGDIR}/${p%-*}/template | cut -d= -f2)"
		...
		done < <(dulge-query -Rf $p | sed 's/ -> .*//')
	done < <(dulge-query -Rs texlive -p pkgver | cut -d : -f 1)
	return $exitcode
}
