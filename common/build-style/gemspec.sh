...
do_build() {
	...
	if [ "$CROSS_BUILD" ]; then
		...
		_TARGET_PLATFORM="$(ruby -r \
			$(find ${DULGE_CROSS_BASE}/usr/lib/ruby -iname rbconfig.rb) \
...
system("sed -i 's|^topdir.*|topdir = ${DULGE_CROSS_BASE}/usr/include/ruby-\$(ruby_version)|' Makefile")
system("sed -i 's|^hdrdir.*|hdrdir = ${DULGE_CROSS_BASE}/usr/include/ruby-\$(ruby_version)|' Makefile")
system("sed -i 's|^arch_hdrdir.*|arch_hdrdir = ${DULGE_CROSS_BASE}/usr/include/ruby-\$(ruby_version)/\$(arch)|' Makefile")
...
EOF
	...
	for f in $distfiles; do
		if [ "${f##*.}" = "gem" ]; then
			$gem_cmd spec \
				"${DULGE_SRCDISTDIR}/${pkgname}-${version}/${f##*/}" \
				--ruby > $gemspec
		fi
	done
