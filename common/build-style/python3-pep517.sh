...
do_build() {
	...
	if [ "${CROSS_BUILD}" ] && [[ "${build_helper}" = *meson* ]]; then
		local mcross="-Csetup-args=--cross-file=${DULGE_WRAPPERDIR}/meson"
		make_build_args+=" ${mcross}/dulge_meson.cross"

		if [[ "${build_helper}" = *numpy* ]]; then
			make_build_args+=" ${mcross}/dulge_numpy.cross"
		fi
	fi
...
do_check() {
	...
	if python3 -c 'import xdist' >/dev/null 2>&1; then
		testjobs="-n $DULGE_MAKEJOBS"
	fi

	local testdir="${wrksrc}/.dulge-testdir/$(date +%s)"
