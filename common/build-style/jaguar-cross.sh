...
_void_cross_build_musl() {
	...
	${tgt}-gcc -pipe -fPIC ${cross_musl_cflags} ${cross_musl_ldflags} -fpie \
		-c ${DULGE_SRCPKGDIR}/${_musl_pkgname}/files/__stack_chk_fail_local.c \
...
do_build() {
	...
	ln -sf usr/lib build_root/lib${DULGE_TARGET_WORDSIZE}
	ln -sf lib build_root/usr/lib${DULGE_TARGET_WORDSIZE}
...
do_install() {
	...
	rm -f ${DESTDIR}/usr/lib${DULGE_TARGET_WORDSIZE}
	rm -f ${DESTDIR}/lib*
	rm -f ${DESTDIR}/*bin
